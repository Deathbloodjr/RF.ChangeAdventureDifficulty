...
using ChangeAdventureDifficulty.Plugins;
using UnityEngine;
using System.Collections;
using SaveProfileManager.Plugins;
using System.Reflection;

namespace ChangeAdventureDifficulty
{
    [BepInPlugin(MyPluginInfo.PLUGIN_GUID, ModName, MyPluginInfo.PLUGIN_VERSION)]
    public class Plugin : BasePlugin
    {
        public const string ModName = "ChangeAdventureDifficulty";
        ...
        public static void LoadPlugin(bool enabled)
        {
            if (enabled)
            {
                ...
                result &= Instance.PatchFile(typeof(ChangeAdventureDifficultyPatch));
                result &= Instance.PatchFile(typeof(SaveScenarioPlay));
                if (result)
                {
